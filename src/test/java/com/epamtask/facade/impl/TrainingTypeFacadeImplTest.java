...
class TrainingTypeFacadeImplTest {
    ...
    void testCreateTrainingType() {
        TrainingTypeEntity type = new TrainingTypeEntity(4L, TrainingType.CROSSFIT);
        when(trainingTypeService.createTrainingType("CROSSFIT")).thenReturn(type);

        TrainingTypeEntity result = trainingTypeFacade.createTrainingType("CROSSFIT");

        assertEquals("CROSSFIT", result.getType().name());
        verify(trainingTypeService).createTrainingType("CROSSFIT");
    }

    @Test
    void testDeleteTrainingTypeById() {
        trainingTypeFacade.deleteTrainingTypeById(5L);
        verify(trainingTypeService).deleteTrainingTypeById(5L);
    }

    @Test
    void testDeleteTrainingType() {
        TrainingTypeEntity type = new TrainingTypeEntity(6L, TrainingType.STRENGTH);
        trainingTypeFacade.deleteTrainingType(type);
        verify(trainingTypeService).deleteTrainingType(type);
    }
}